...
export const createModule = <I extends EmptySet, O extends EmptySet>(
  ...
  const inlets: PdElement<EmptySet, EdgeSet>[] = inletNames.map(name =>
    streamPortRegex.test(name) ? Inlet$() : Inlet()
  )
  const outlets: PdElement<EdgeSet, EmptySet>[] = outletNames.map(name =>
    streamPortRegex.test(name) ? Outlet$() : Outlet()
  )
...
export const createMainModule = (
  moduleCtor: ModuleConstructor<EmptySet, EmptySet>
) => createModule("main", [], [], moduleCtor)
...
const parseConnections = (elements: PdElement<any, any>[]): PdConnection[] =>
  unnest(
