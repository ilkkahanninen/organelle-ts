...
export const createOrganelleFileClient = (deviceAddress: string) => {
  const call = async <T extends object>(path: string, query?: object, options?: RequestInit): Promise<T> => {
    const q = query
  ...
  const getNodes = (path: string) => call<FSNode[]>("/files/fmdata", {
    operation: 'get_node',
    path
  })

  const createFolder = (parentDir: string, name: string) => call("/files/fmdata", {
    operation: 'create_node',
    path: parentDir,
    name: name
  })

  const deleteNode = (path: string) => call("/files/fmdata", {
  ...
  const upload = async (path: string, filename: string) => {
    ...
    form.append('dst', path.split('/').filter(n => n).join('/') + '/')
  ...
  const uploadFiles = async (path: string, filenames: string[]) => {
    ...
    const existingNodes = (await getNodes(path)).filter(node => basenames.includes(node.name))
