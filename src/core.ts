...
const connectablesToPortMappings = (
  connectables?: Connectables
): PortMapping[] =>
  ensureArray(connectables).map(connectable =>
    isPortMapping(connectable)
      ? connectable
...
export const withCreatePdElementHook = (
  hook: CreatePdElementHook,
  context: () => void
) => {
...
const createPdElement = <I extends EmptySet, O extends EmptySet>(
  ...
  const connectablesToConnections = (
    connectables: ConnectablesMap<I> | Connectables
  ): Connection[] =>
    isConnectablesMap(connectables)
      ? unnest(
          Object.keys(connectables).map((inletName: I[number]) => {
            ...
            if (portIndex < 0) {
              ...
              throw Error(
                `Unexpected error: Could not connect to unknown inlet '${inletName}'`
              )
            }
            const target = { element, portIndex }
            return connectablesToPortMappings(connectables[inletName]).map(
              source => ({ source, target })
            )
          })
        )
      : connectablesToPortMappings(connectables).map(source => ({
          source,
          target: { element, portIndex: 0 }
        }))
...
export const msg = (
  value: Value,
  inlets: Connectables
): PdElement<typeof msgInlets, typeof msgOutlets> =>
  createPdElement("msg", value, { trigger: inlets }, msgInlets, msgOutlets)

export const objCreator = <A extends EmptySet, B extends EmptySet>(
  name: string,
  inletNames: A,
  outletNames: B
) => (
  inlets: ConnectablesMap<A> | Connectables = {},
  value: Value = ""
): PdElement<A, B> =>
  createPdElement("obj", toCtor(name, value), inlets, inletNames, outletNames)

export const objCreator2 = <A extends EmptySet, B extends EmptySet>(
  name: string,
  inletNames: A,
  outletNames: B
) => (
